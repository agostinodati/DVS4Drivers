...
def main_optical_flow():
    with AedatFile(amal1) as f:
            ...
            for e in packet.tolist():
                ...
                if ts >= prev_videoframe_ts + video_dt:
                    while video_frame.timestamp <= ts:
                        ...
                        new_landmarks_true, is_video = dvs4d_lib.find_landmarks(video_frame.image, accumulator_frame,
                                                                                inverse_order=True)
                        fail_counter += 1
        ...
        pl.savefig('errorplot-DT_' + str(event_dt) + '-INCREMENT_' + str(accum_increment) + '-ACCUMTS_' + str(accum_dt))
        pl.show()
